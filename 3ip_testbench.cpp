...
bool check_nat_funtionality(std::vector<Packet>& packets, std::vector<Packet>& recv_packets) {
    ...
    for (int i = 0; i < packets.size(); i++) {
        ...
        if (nat_table.find(tuple5) == nat_table.end()) {
            ...
            // special guarantee
            if (nat_table.size() != ntohs(recv_tuple5.src_port)) {
                printf("new connection mismatch: nat_table.size() != ntohs(recv_tuple5.src_port)\n");
                printf("packet idx = %d\n", i);
                printf("nat_table.size() = %ld\n", nat_table.size());
                printf("recv_tuple5.src_port = %d\n", recv_tuple5.src_port);
                printf("ntohs(recv_tuple5.src_port) = %d\n", ntohs(recv_tuple5.src_port));
